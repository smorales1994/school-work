...
public class ColorMappingColorChooser implements ColorChooser
{
	...
	public void createIterSpectrumMap()
	{
		...
		for (int keys: iterCountMap.keySet())
		{
			iterSpectrumMap.put(keys, sum + (iterCountMap.get(keys) / 2) + 1);

			sum += iterCountMap.get(keys);
		}
	...
	{
		for (int keys: iterSpectrumMap.keySet())
		{
			if (keys == Mandelbrot.THRESHOLD)
			{
				iterColorMap.put(keys, Color.BLACK);
			}

			else
			{
				double location = (double) iterSpectrumMap.get(keys) / maxLocation;
				double r = Math.sin(location * (Math.PI / 2)) * 255;
				double g = Math.sin(location * Math.PI) * 255;
				double b = Math.cos(location * (Math.PI / 2)) * 255;

				Color color = new Color((int) r, (int) g, (int) b);

				iterColorMap.put(keys, color);
			}
		}
	}
}
